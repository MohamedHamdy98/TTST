...
def predict(
    prompt, language, audio_file_pth, mic_file_path, use_mic, voice_cleanup, no_lang_auto_detect, agree):
    if agree == True:
...
def process_audio_button(audio_file, language):
    if audio_file is None:
        return "No audio file provided"
    return file_to_text(recognizer, language, audio_file)    
...
with gr.Blocks(analytics_enabled=False) as demo:
    ...
    with gr.Row():
        with gr.Column():
            ...
            language_translation_gr = gr.Dropdown(
                label="Language Translation",
                info="Select an output language for the Translation Text",
                choices=[
                    "en",
                    "es",
                    "fr",
            ...
            button_sst = gr.Button("Send Audio", elem_id="send-stt-btn", visible=True)
            button_translation = gr.Button("Translation", elem_id="send-translation-btn", visible=True)
            tts_button = gr.Button("Send", elem_id="send-btn", visible=True)

        with gr.Column():
            # video_gr = gr.Video(label="Waveform Visual")
            output_text_gr = gr.Textbox(label="Transcribed Text")
            output_translation_gr = gr.Textbox(label="Translation Text")
    ...
    #                 label="Examples",
    #                 inputs=[prompt, language_gr, ref_gr, mic_gr, use_mic_gr, clean_ref_gr, auto_det_lang_gr, tos_gr],
    #                 outputs=[video_gr, audio_gr, out_text_gr, ref_audio_gr],
    #                 fn=predict,
    #                 cache_examples=False,)

    button_sst.click(process_audio_button, inputs=[ref_gr_test, language_gr], outputs=output_text_gr)
    button_translation.click(translate_text, inputs=[output_text_gr, language_gr, language_translation_gr], outputs=output_translation_gr)
    tts_button.click(predict, [prompt, language_gr, ref_gr, mic_gr, use_mic_gr, clean_ref_gr, auto_det_lang_gr, tos_gr], outputs=audio_gr)
